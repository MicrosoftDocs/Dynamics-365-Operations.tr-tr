...
<a name="what-is-the-goal-of-sepa-direct-debits"></a>SEPA doğrudan alacakların amacı nedir?
---------------------------------------

Alacaklı için müşteri tarafından imzalanmış zorunluluğuna verilmiş olması koşuluyla SEPA Talimatlı Alacaklıya bir müşterinin banka hesabından fon toplamak izin verir. Müşteri, alacaklının tahsilat yapmasına izin veren bir yönerge imzalar ve müşteri bankasına ödemeyi yapması doğrultusunda talimat verir. 
...
Uluslararası Banka hesap numarası (IBAN) ve banka tanımlayıcı kodu (BIC) 32 SEPA ülkelerde/bölgelerde herhangi bir hesabı tanımlamak için kullanılır. BIC SWIFT kodu alanını ve IBAN IBAN alanına girin. Her iki alan da Banka hesapları sayfasının Banka hesabı sekmesindeki Ek Kimlik hızlı sekmesinde bulunur. Bu hem alacaklı banka hesabı hem de müşteri banka hesabı için geçerlidir.
...
SEPA veri biçimleri ISO 20022 ileti standartlarını temel alır. Sizin genel elektronik raporlama onay kutusunu işaretleyin ve Alacak hesapları ödeme yöntemlerini yapılandırırken bir dışa aktarma biçimi yapılandırma SEPA doğrudan borç biçimi seçin. Müşteri ödeme günlüğünde ödeme dosyası oluşturduğunuzda, bu ödeme yöntemini kullanırsınız.
