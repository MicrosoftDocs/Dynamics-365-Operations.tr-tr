...
Yalnızca tek bir giriş varsa, tüm çıkışlar bundan kapatılabilir ve sanal transfer oluşturulamaz. Ağırlıklı ortalamayı kullandığınızda, ağırlıklı ortalama kuralını kullanmak yerine, stok hareketlerini belirli bir madde girişinin belirli bir çıkışa karşı kapatılmasını sağlayacak şekilde işaretleyebilirsiniz. Ağırlıklı ortalama stok modelini kullandığınızda aylık stok kapanışını kullanmanızı öneririz. Ağırlıklı ortalama stok maliyetlendirme yöntemine aşağıdaki formülle hesaplanır:
-   Ağırlıklı ortalama = (S1\*P1 + S2\*P2 + Qn\*Pn) / (S1 S2 + Qn)

Stok çıkışlarını terk eden stok hareketleri. Buna satış emirleri, stok günlükleri ve üretim emirleri dahildir, deftere nakil tarihinde tahmini bir maliyet fiyatında gerçekleşir. Bu tahmini maliyet fiyatına cari ortalama da denir. Stok kapatma zaman sistem geçerli ve önceki dönemlere ait stok hareketlerini incelemek ve aşağıdaki kurallara kapanış hangisinin kullanılması gerektiğini belirlemek.
-   Doğrudan kapatma
-   Özetlenmiş kapatma
...
Doğrudan Kapatma ilkesini Ağırlıklı ortalama önceki sürümlerinde kullanılan aynıdır. Sistem doğrudan giriş ve çıkışlar arasında kapatır. Sistem belirli bazı durumlarda bu doğrudan Kapatma ilkesini kullanır:
...
Aşağıdaki senaryoda, mali olarak güncelleştirilmiş giriş ve çıkış deftere nakledilmiştir. Stok kapatma sırasında sorunundan doğrudan giriş sistemi kapatıp kapatmayacağını ve sorunla ilgili hiçbir maliyet fiyatı düzeltmesi gerekiyor. Aşağıdaki hareketler grafikte gösterilmektedir.
...
-   Each vertical arrow is labeled with a sequential identifier, such as *1a*. Tanımlayıcılar, zaman çizgisindeki hareket nakillerinin sırasını belirtir.
...
Fiziksel değeri dahil et parametresini farklı daha Ağırlıklı ortalama stok modeli önceki ürün sürümlerinde çalışır. Madde modeli grubu formunda bir madde için Fiziksel değeri dahil et kutusunu seçin. Sistemin fiziksel olarak güncelleştirilen giriş tahmini maliyet fiyatını hesaplarken kullanacağı sonra veya ortalama çalışan. Çıkışlar dönem sırasında bu tahmini maliyet fiyatına dayanarak deftere nakledilir. Stok kapanışı sırasında mali olarak güncelleştirilmiş girişler yalnızca ağırlıklı ortalama hesaplamasında hesaba katılır. Ağırlıklı ortalama stok modelini kullanırken aylık bir stok kapanışı öneririz. Bu ağırlıklı ortalama doğrudan kapatma örneğinde, madde modeli grubu fiziksel değeri içerecek şekilde işaretlenmiştir. Aşağıdaki hareketler aşağıdaki grafikte gösterilmektedir:
...
INCLUDE fiziksel değer parametre farklı Ağırlıklı ortalama önceki sürümlerinde çalışır. Madde modeli grubu sayfasında bir madde için Fiziksel değeri dahil et kutusunu seçin. Daha sonra sistem fiziksel olarak güncelleştirilen giriş tahmini maliyet fiyatı veya çalışan ortalaması hesaplamasında kullanır. Çıkışlar dönem sırasında bu tahmini maliyet fiyatına dayanarak deftere nakledilir. Stok kapanışı sırasında mali olarak güncelleştirilmiş girişler yalnızca ağırlıklı ortalama hesaplamasında hesaba katılır. Ağırlıklı ortalama stok modelini kullanırken aylık bir stok kapanışı öneririz. Bu ağırlıklı ortalama özetlenmiş kapatma örneğinde, stok modeli grubu fiziksel değeri içerecek şekilde işaretlenmiştir. Aşağıdaki hareketler aşağıdaki grafikte gösterilmektedir:
-   1a. Her biri 11,00 ABD Doları maliyetinde 2 miktarı için güncelleştirilen stok fiziksel girişi.
-   1b. Her biri 14,00 ABD Doları maliyetinde 2 miktarı için güncelleştirilen stok mali girişi.
