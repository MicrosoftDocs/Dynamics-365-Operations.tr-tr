...
| Madde satış rezervasyonu  | Madde rezervasyonunun el ile veya otomatik olduğunu belirler. Rezervasyon otomatikse stok, sipariş satırları oluşturulduğunda rezerve edilir. Rezervasyonları ürün reçeteleri için madde numarası düzeyinde (**Otomatik** seçeneği) veya bir ürün reçetesinin ayrı ayrı öğeleri için (**Açılım** seçeneği) yapmak mümkündür. Varsayılan değeri **madde satış rezervasyon** devralındığı **Accounts receivable parameters.** Bu sayfada Rezervasyon alanında bulunan değeri ayarlanmış **satış varsayılan değeri****bölüm** üzerinde **genel** sekme. |
