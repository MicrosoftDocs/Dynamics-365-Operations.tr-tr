...
-   Sütun tanımının üstbilgiler alanı, raporda görüntülenecek üstbilgi metnini ve biçimlendirmeyi içerir. Üstbilgi tek bir veri sütunu için uygulayabilirsiniz, birden çok sütuna yayılan veya koşullu ayrı ayrı sütunlara uygulayabilirsiniz. Sütun tanımı, gereksinim duyduğunuz kadar çok sütun başlık satırları içerebilir. **Not:** Sütun üstbilgileri, raporun her bir veri sütununa uygulanır. Rapor üstbilgileri tüm rapora uygulanır. Rapor üstbilgilerini, rapor tanımının **Üstbilgiler ve Altbilgiler** sekmesinde tanımlarsınız.
...
|-------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| SU                      | Bir sütun için alt çizgi alt çizgi komutu nerede bastırmak (**---**) ya da çift alt çizgi komutu (**===**) satır tanımı girilir. Örneğin, bir yüzde hesaplaması tarafından üretilen tutarları alt çizgili görüntülemek istemeyebilirsiniz.                                                                        |
...
| Dönem sayısı (@FiscalPeriod)           | İlgili sütun için tanımlanan mali dönemin sayısını yazdırın. Sütun birden fazla dönemi kapsıyorsa aralıktaki son dönem yazdırılır.                                                                                                                                   |
| Dönem açıklaması (@FiscalPeriodName)  | Mali verilerde açıklanan mali dönem açıklamasını yazdırın.                                                                                                                                                                                                                    |
| Mali yıl (@FiscalYear)               | Sütun için mali yılı sayısal formatta yazdırın.                                                                                                                                                                                                                                            |
...
| Birim adı ağacından (@UnitName)         | Bir sütunu raporlama ağacının belirli bir birimiyle sınırlandırırsanız birim adını sütun üstbilgisinde yazdırın.                                                                                                                                                                                     |
| Birim açıklaması (@UnitDesc)            | Bir sütunu raporlama ağacının belirli bir birimiyle sınırlandırırsanız birim açıklamasını sütun üstbilgisinde yazdırın.                                                                                                                                                                              |
| Kitap kod (@BookCode)                   | Sütunda tanımlanan defter kodunu yazdırın.                                                                                                                                                                                                                                             |
...
-   Çakışan sütun üstbilgileri tanımlarsanız, bunlar raporda yazdırıldığında üst üste gelir. Rapor oluşturulur, ancak aşağıdaki uyarı görünür **rapor sırası durumu** alan: "Base kullanarak sütun başlıkları ile diğer sütun başlıklarını kesiştiği ve üst üste gelen metin neden olabilir." Örneğin, B sütununun başlığı tanımını B temel + 1'e ve D sütun üstbilgisi tanımını temel + F. 1 Bu durumda, üstbilgileri birbirinin üstüne yazdırılır ve okunamaz durumda. Bir **Üstbilgiyi başlat/Üstbilgiyi sonlandır** tanımında TEMEL kullanılıyorsa, üstbilgilerin çakışıp çakışmadığını kontrol etmek için, oluşturulan rapor görüntülediğinizden emin olun.
...
-   Koşullu baskı sütunlar için (**P&lt;B**, **P&gt;= B**), koşullu üstbilgi yayılan herhangi normal sütun başlığı tanımı gibi davranır. Örneğin, koşul yanlış ise, genişleme koşulu için daha sonraki bir sütun eşleşmesi, üstbilginin yazdırılmasını başlatır.
...
3.  Listeden bir sütun türü seçin. Aşağıdaki tabloda çeşitli sütun türleri açıklanmıştır.
    ...
    <tr class="odd">
    <td>ATTR</td>
    <td>Muhasebe sisteminiz öznitelikleri destekliyorsa, sütunda bir hesap veya hareket özniteliği görüntülenir. Bir tekli tam hesaba uygulanması gereken bir öznitelik, mali verilerden altı çizili hesap veya hareket bilgilerini çeker. Hesap düzeyi nitelikler hesap verileri görüntüler ve işlem düzeyi nitelikler hareketinin nakledildiği sırada oluşan verileri görüntüleme. Seçerseniz <strong>ATTR</strong> özniteliği kategorisinde sütun türü olarak belirtmek <strong>defteri kod/özniteliği kategori</strong> ayrıntı satır sütun tanımının.</td>
    </tr>
    </tbody>
    </table>
...
| TEMEL +\# | Olan yılını kullan \#temel yıldan sonraki yıl. Örneğin, temel yıldan sonraki üçüncü yılı kullanmak istiyorsanız **TEMEL+3** girin. |
| BASE-\# | Olan yılını kullan \#yıl önce temel yıl. Örneğin, geçen yılı kullanmak için **TEMEL-1** girin.                 |
...
| TEMEL +\#         | Dönemi kullanın \#temel süresinden sonra nokta. Örneğin, temel dönemden sonraki üçüncü dönemi kullanmak istiyorsanız **TEMEL+3** girin.                                                                                               |
| BASE-\#         | Dönemi kullanın \#temel dönemden önceki dönemler. Örneğin, geçen dönemi kullanmak için **TEMEL-1** girin.                                                                                                                 |
| BASE -\#: temel    | Temel dönemden önceki birkaç dönemden temel döneme kadar, birden fazla dönem kullanılır. Örneğin, önceki üç dönemi ve temel dönemi kullanmak için **TEMEL-3:TEMEL** girin.                                                |
...
| BASE -\#: temel +\# | Temel dönemden önceki birkaç dönemden temel dönemden sonraki birkaç döneme kadar, birden fazla dönem kullanılır. Örneğin, önceki üç dönemi, temel dönemi ve sonraki iki dönemi kullanmak için **TEMEL-3:TEMEL+2** girin. |
...
Dönem özelliklerinin herhangi birinde mali yıl sınırlarının ötesine geçebilir ve bir dönem aralığındaki yılları karıştırabilirsiniz. Örneğin, nokta olarak belirttiğiniz **BANKASI-5** (Geçmiş temsil etmek için altı dönemi) ve 2'in temel nokta olan bir raporu çalıştırma. Bu durumda, raporda belirtilen mali yılın ilk iki dönem ve önceki mali yılın son dört dönemler için veri gösterilir.
...
3.  Gezinme çubuğunun üzerindeki formül çubuğunda veya **Dönem** hücresinde, formülü tamamlayın. Herhangi bir sayı işareti Değiştir (\#) uygun değer ile.
...
| Sütun Genişliği                 | 30   |                      |
| ...                          |      |                      |
| Öznitelik Filtresi             |      |  Başvuru =\[01:10\] |

### <a name="dimension-filter-in-a-column-definition"></a>Bir sütun tanımındaki boyut filtresi

Bir boyut filtresi, **FD** sütununu belirli boyut değerleriyle sınırlandırmak için kullanılır. Filtre tek bir boyut, bir boyut aralığı veya bir boyut grubu içerebilir. Filtre de boyut değer kümeleri içerebilir. Boyut değerleri değişebilir çünkü bir... \financial-dimensions\dimension-based sistemi için tam bir uzunluk karşılık gerekmez. Raporun bir raporlama ağacı içerip içermediğinden bağımsız olarak filtre uygulanır. Bir joker karakter kullanabilirsiniz (\* veya?) herhangi bir konumda. Birden fazla hesap belirttiğinizde, aşağıdaki örnekte gösterildiği gibi hesaplar arasında virgül koymak: + hesap =\[1200\], + hesap =\[1100\], departman =\[01?\] Belirli bir hesap için tüm departmanları almak için, boyut filtresinden Departman boyutunu çıkarabilirsiniz. Örneğin, aşağıdaki boyut filtrelerinin her ikisi de aynı şekilde işlenir:

-   + Hesap =\[1100\], departman
-   + Hesap =\[1100\]

Ayrıca, tam eşleştirme için alfasayısal karakterlerin herhangi bir kombinasyonunu kullanabilir ve kısmi boyutlar tanımlayabilirsiniz. Örneğin, **konum = \[10\*\]** başlayan tüm konum olan boyut değerleri 10 içerir.
...
-   **Para birimi görüntülemeyi** – (doğal, işlev veya Raporlama) para birimi türünü belirtin hareketleri görüntülenecek. Bu işlevsellik, para birimi çeviri bazen denir. Para birimi çevirisi, genel muhasebe tutarlarının şirketin işlevsel para birimi veya girilen hareketin para birimi dışında bir para biriminde rapor edilebilmesi anlamına gelir.
...
**Not:** Burada açıklanan seçenekler, ERP sistemine bağlı olarak farklılık gösterebilir. Daha fazla bilgi için [Microsoft ERP sistemi belgeleri](https://www.microsoft.com/en-us/download/details.aspx?id=5916).
...
| Şirket bilgilerinden işlevsel para birimi | Boş                | **$2,310\*\*** – tüm veriler sonucu Şirket bilgilerinde belirtilen işlev para birimi cinsinden gösterilir. **Not:** Bu tutar, tüm hareketlerin işlevsel para birimindeki toplamıdır. |
