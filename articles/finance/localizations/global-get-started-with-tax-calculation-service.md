...
3. Finance ve Supply Chain Management'ta tüzel kişiliğe göre vergi hesaplama eklentisi parametrelerini ayarlayın.
...
- LCS hesabınıza erişiminiz var ve bir LCS projesini Dynamics 365 sürüm 10.0.18 veya sonrasını çalıştıran bir katman 2 (veya daha sonrası) ortamıyla dağıttınız.
...
## <a name="set-up-the-tax-calculation-add-in-in-lcs"></a>LCS'de vergi hesaplama eklentisini ayarlama
...
4. **Vergi hesaplamasını (Önizleme)** seçin.
5. Hüküm ve koşulları okuyup kabul edin ve sonra **Yükle**'yi seçin.

## <a name="set-up-the-tax-calculation-add-in-in-rcs"></a>RCS'de vergi hesaplama eklentisini ayarlama
...
6. **Aç**'ı seçin.
7. **Vergi veri modeline** gidin, dosya ağacını genişletin ve ardından **vergi konfigürasyonu-Avrupa**'yı seçin.
...
17. Vergi kodunun niteliğini tanımlamak için aşağıdaki seçenekleri kullanın:
    ...
    - Kullanım vergisi
    - Karşı ödeme
    - Temel tutar hesaplamasında hariç tut
    ...
    Bazı ülkelerde gümrük vergisi gibi fiyat dahil işlemler için vergi matrahı hesaplamasında hariç tutulması gereken bazı vergi türleri için, **temel tutar hesaplamasında hariç tut** onay kutusunu seçin.
...
RCS'deki ayarlamayı tamamladıktan sonra, önceki bölümde açıklandığı gibi vergi özelliğinin yayınlanmış bir sürümüne sahip olacaksınız. Finance'te vergi hesaplama eklentisibi ayarlamak için bu adımları izleyin.

Bu bölümdeki ayarlama tüzel kişilik tarafından gerçekleştirilir. Finance'te Vergi hesaplama eklentisini etkinleştirmek istediğiniz her tüzel kişilik için yapılandırmalısınız.

1. Finance'te, **Vergi** \> **Ayarla** \> **Vergi yapılandırması** \> **Vergi hesaplama eklentisi ayarı (Önizleme)** bölümüne gidin.
2. **Genel** sekmesinde, aşağıdaki alanları ayarlayın:

    - **Vergi hesaplama eklentisini Etkinleştir**:Tüzel kişilik için vergi hesaplama eklentisini etkinleştirmek için bu onay kutusunu işaretleyin. Vergi hesaplama eklentisi geçerli tüzel kişilik için etkinleştirilmemişse, tüzel kişilik vergiyi belirlemek ve hesaplamak için mevcut vergi altyapısını kullanmaya devam eder.
    ...
    - **İş süreci**: Vergi hesaplama eklentisi için etkinleştirilecek iş süreçlerini seçin.
...
4. **Hata işleme** sekmesinde, tüzel kişilik için beklenen hata işleme yöntemini tanımlayın. Vergi hesaplama eklentisinden her bir sonuç kodu için üç seçenek vardır:

    - No
    - Uyarı
    - Hata

5. Vergi hesaplama eklentisini ayarını kaydedin.
6. Her ek tüzel kişilik için 1 ile 5 arasındaki adımları yineleyin.

## <a name="transaction-processing"></a>İşlem işleme

Tüm ayarlama yordamlarını tamamladıktan sonra, Finance'te vergiyi belirlemek ve hesaplamak için vergi hesaplama eklentisini kullanabilirsiniz. İşlemleri işleme adımları aynı kalır. Aşağıdaki işlemler Finance sürüm 10.0.18'de desteklenmektedir:
