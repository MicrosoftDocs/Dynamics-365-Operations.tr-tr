...
Mali içgörüler, kuruluşunuza güçlü tahmin araçları sunmak için Microsoft Dynamics 365 Finance işlevlerini Common Data Service, Azure ve AI Builder işlevleriyle bir araya getirir. Bu konuda, sisteminizin Mali içgörülerde sunulan özellikleri kullanabilmesini sağlayacak yapılandırma adımları açıklanmaktadır.
...
## <a name="configure-common-data-service"></a>Common Data Service'ı yapılandırma
...
1. [Power Platform yönetim merkezini](https://admin.powerplatform.microsoft.com/)açın ve aynı Active Directory kiracısında yeni bir Common Data Service ortamı oluşturmak için aşağıdaki adımları izleyin:
    ...
    11. Common Data Service kuruluş kodunu not edin.
    ...
    16. **Kurulum Referansı Bilgi Kodu** alanını, daha önce not ettiğiniz Common Data Service kuruluş kodu değeri olarak ayarlayın.
    17. Tarayıcının adres çubuğunda Common Data Service kuruluşunun URL'sini not edin. Örneğin, URL şu şekilde olabilir: `https://org42b2b3d3.crm.dynamics.com`.
...
### <a name="enter-the-common-data-service-directory-id-and-the-users-azure-ad-object-id"></a>Common Data Service dizin kodunu ve kullanıcının Azure AD nesne kodunu girin

1. Common Data Service dizin kodunu girin:

    1. [Azure portalını](https://portal.azure.com) açın.
    2. Common Data Service ortamı oluşturmak için kullanılan kullanıcı kimliğini kullanarak oturum açın.
...
> [!NOTE]
> Common Data Service ortamıyla aynı Azure AD kurulumunda aşağıdaki kaynakları oluşturduğunuzdan emin olun. Farklı bir Azure AD kurulumundan kaynak kullanamazsınız.
...
4. **İçgörü edinme** eklenti ayrıntıları sayfasında aşağıdaki değerleri girin.
    ...
    | Değer                                                    | Tanım |
    |----------------------------------------------------------|-------------|
    | CDS Kuruluş URL'si                                     | Common Data Service kurulumunun Common Data Service kuruluşu URL'si. Bu değeri bulmak için [Power Apps portalını](https://make.powerapps.com) açın, sağ üst köşede **Ayarlar** düğmesini (dişli simgesi) seçin, **Gelişmiş ayarlar**'ı seçin ve URL'yi kopyalayın. (URL "dynamics.com" ile biter) |
    | CDS Kuruluş Kimliği                                               | Common Data Service kurulumunun ortam kimliği. Bu değeri bulmak için [Power Apps portalını](https://make.powerapps.com) açın, sağ üst köşede **Ayarlar** düğmesini (dişli simgesi) seçin, **Özelleştirmeler \> Geliştirici Kaynakları \> Kurulum Referansı Bilgileri** öğesini seçin ve **Kimlik** değerini kopyalayın. |
    | CDS Kiracı Kimliği (AAD'den Dizin Kimliği)               | Common Data Service kurulumunun kiracı kimliği. Bu değeri bulmak için [Azure portalı](https://portal.azure.com) açın , **Azure Active Directory** öğesine gidin ve **Kiracı Kimliği** değerini kopyalayın. |
    | Sistem yöneticisi rolüne sahip kullanıcı nesnesi kimliğini belirtin | Common Data Service'te kullanıcının Azure AD kullanıcı nesnesi kimliği. Bu kullanıcı, Common Data Service kurulumunun sistem yöneticisi olmalıdır. Bu değeri bulmak için, [Azure portalı](https://portal.azure.com) açın, **Azure Active Directory \> Kullanıcılar** bölümüne gidin, kullanıcıyı seçin ve **Kimlik** bölümünde **Nesne Kimliği** değerini kopyalayın. |
    | Bu kiracı için varsayılan CDS ortamı mı?      | Common Data Service kurulumu, oluşturulan ilk üretim kurulumu ise bu onay kutusunu seçin. Common Data Service kurulumu el ile oluşturulmuşsa bu onay kutusunun işaretini kaldırın. |
