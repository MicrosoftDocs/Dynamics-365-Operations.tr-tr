...
<span data-ttu-id="3c595-105">Recency, sıklık ve parasal (RFM) analiz, organizasyonunuzun Müşteri Harcamaları tarafından üretilen verileri değerlendirmek için kullanabileceği bir pazarlama aracıdır .</span><span class="sxs-lookup"><span data-stu-id="3c595-105">Recency, frequency, and monetary (RFM) analysis is a marketing tool that your organization can use to evaluate the data that is generated by customer purchases.</span></span> <span data-ttu-id="3c595-106">RFM analiz ayarladıktan sonra müşterilere Satın almalar yaparken hesaplanan bir RFM puan atanır.</span><span class="sxs-lookup"><span data-stu-id="3c595-106">After you set up RFM analysis, customers are assigned a calculated RFM score as they make purchases.</span></span> <span data-ttu-id="3c595-107">RFM puanı üç basamaklı derecelendirme veya kuruluşunuzun RFM analiz nasıl yapılandırdığına bağlı olarak bir toplama numarası olabilir.</span><span class="sxs-lookup"><span data-stu-id="3c595-107">The RFM score can be a three-digit rating or an aggregate number, depending on how your organization has configured RFM analysis.</span></span> <span data-ttu-id="3c595-108">Kuruluşunuzun puanlama için üç basamaklı derecelendirme kullanması durumunda derecelendirme aşağıdaki şekilde çalışır:</span><span class="sxs-lookup"><span data-stu-id="3c595-108">Here’s how the rating works if your organization uses a three-digit rating for the score:</span></span>

- <span data-ttu-id="3c595-109">İlk basamak müşterinin recency derecesidir. Bu derece müşterinin kuruluşunuzdan en son ne zaman satınalma yaptığını gösterir.</span><span class="sxs-lookup"><span data-stu-id="3c595-109">The first digit is the customer's recency rating, which is how recently the customer made a purchase from your organization.</span></span> 
- <span data-ttu-id="3c595-110">İkinci basamak müşteri sıklık derecesidir. Bu, genellikle müşterinin kuruluşunuzdan ne sıklıkla satınalma yaptığını gösterir.</span><span class="sxs-lookup"><span data-stu-id="3c595-110">The second digit is the customer's frequency rating, which is how often the customer makes purchases from your organization.</span></span> 
- <span data-ttu-id="3c595-111">Üçüncü basamak müşterinin parasal derecelendirmesidir. Bu, müşterinin kuruluşunuzdan yaptığı satınalımlarda ne kadar para harcadığını gösterir.</span><span class="sxs-lookup"><span data-stu-id="3c595-111">The third digit is the customer's monetary rating, which is how much the customer spends when he makes purchases from your organization.</span></span> 

<span data-ttu-id="3c595-112">Örneğin, kuruluşunuzun derecelendirme 1 ile 5 arası bir ölçek üzerinde 5 en yüksek derece olarak ayarlamıştır.</span><span class="sxs-lookup"><span data-stu-id="3c595-112">For example, your organization has set the ratings on a scale of 1 through 5, where 5 is the highest rating.</span></span> <span data-ttu-id="3c595-113">Bu durumda, bir müşteri 535 derecesi müşteri hakkında aşağıdaki bilgileri gösterir:</span><span class="sxs-lookup"><span data-stu-id="3c595-113">In this case, a customer rating of 535 tells you the following information about the customer:</span></span>

-   <span data-ttu-id="3c595-114">**5 recency derecesi** – müşteri yakın zamanda satın alma yapmıştır.</span><span class="sxs-lookup"><span data-stu-id="3c595-114">**Recency rating of 5** – The customer recently made a purchase.</span></span>
-   <span data-ttu-id="3c595-115">**Sıklık derecesi 3** – müşteri oldukça sık, organizasyonunuzun ürünleri satın almaktadır.</span><span class="sxs-lookup"><span data-stu-id="3c595-115">**Frequency rating of 3** – The customer purchases products from your organization moderately often.</span></span>
-   <span data-ttu-id="3c595-116">**Parasal derecesi 5** – Müşteri bir satın alma yaptığında önemli miktarda para harcar.</span><span class="sxs-lookup"><span data-stu-id="3c595-116">**Monetary rating of 5** – When the customer makes a purchase, he spends a significant amount of money.</span></span>

<span data-ttu-id="3c595-117">Kuruluşunuz için puan toplama bir numarası kullanıyorsa, bireysel derecelendirme birlikte eklenir.</span><span class="sxs-lookup"><span data-stu-id="3c595-117">If your organization uses an aggregate number for the score, the individual ratings are added together.</span></span> <span data-ttu-id="3c595-118">Aynı örnekte, müşterinin derecesi 13 (5 + 3 + 5).</span><span class="sxs-lookup"><span data-stu-id="3c595-118">For the same example, the customer has a rating of 13 (5 + 3 + 5).</span></span>

## <a name="to-set-up-rfm-analysis-for-the-customers-in-your-organization"></a><span data-ttu-id="3c595-119">Kuruluşunuzdaki müşteriler için RFM analizini ayarlamak için.</span><span class="sxs-lookup"><span data-stu-id="3c595-119">To set up RFM analysis for the customers in your organization.</span></span>

1.  <span data-ttu-id="3c595-120">**Çağrı merkezi** > **Periyodik** > **RFM analizi**'ne gidin.</span><span class="sxs-lookup"><span data-stu-id="3c595-120">Go to **Call center** > **Periodic** > **RFM analysis**.</span></span>

2.  <span data-ttu-id="3c595-121">**RFM analizi** sayfasında, **Yeni**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="3c595-121">On **RFM analysis** page, select **New**.</span></span> <span data-ttu-id="3c595-122">**RFM tanımı** alanına, RFM tanımıyla ilgili adı girin.</span><span class="sxs-lookup"><span data-stu-id="3c595-122">In the **RFM definition** field, enter a name for the RFM definition.</span></span> <span data-ttu-id="3c595-123">Örneğin, tanımı RFM-A olarak adlandırabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="3c595-123">For example, you could call the definition RFM-A.</span></span>

3.  <span data-ttu-id="3c595-124">Bu RFM tanımı için bir başlangıç tarihi ve bitiş tarihi girin.</span><span class="sxs-lookup"><span data-stu-id="3c595-124">Enter a start date and end date for this RFM definition.</span></span>

4.  <span data-ttu-id="3c595-125">**Genel** hızlı sekmesinde, aşağıdakileri yapın:</span><span class="sxs-lookup"><span data-stu-id="3c595-125">On the **General** FastTab, do the following:</span></span> 
...
1.  <span data-ttu-id="3c595-156">**Çağrı merkezi** > **Günlükler** > **Müşteri hizmetleri**'ne gidin.</span><span class="sxs-lookup"><span data-stu-id="3c595-156">Go to **Call center** > **Journals** > **Customer service**.</span></span> 

2.  <span data-ttu-id="3c595-157">**Müşteri Hizmetleri** bölmesindeki **Müşteri hizmetleri** sayfasında, arama alanlarında arama için kullanacağınız anahtar sözcüğü yazmayı seçin ve arama metnini girin.</span><span class="sxs-lookup"><span data-stu-id="3c595-157">On the **Customer service** page, in the **Customer service** pane, in the search fields, select the keyword type to search on and enter the search text.</span></span>

3.  <span data-ttu-id="3c595-158">**Ara**'yı seçin.</span><span class="sxs-lookup"><span data-stu-id="3c595-158">Select **Search**.</span></span>

4.  <span data-ttu-id="3c595-159">**Müşteri arama** sayfasında istediğiniz müşterinin kaydını seçin ve ardından **Müşteri seç**'e tıklayın.</span><span class="sxs-lookup"><span data-stu-id="3c595-159">On the **Customer search** page, select the customer record that you want, and then click **Select customer**.</span></span> 

<span data-ttu-id="3c595-160">RFM puanı **Müşteri Hizmetleri** sayfasının sağ tarafındaki **Sipariş geçmişi** grubunda görüntülenir.</span><span class="sxs-lookup"><span data-stu-id="3c595-160">The RFM score is displayed in the **Order history** group on the right side of the **Customer service** page.</span></span> 

## <a name="view-or-clear-the-history-of-an-rfm-analysis-record"></a><span data-ttu-id="3c595-161">RFM analizi kaydı geçmişini görüntüleme veya silme</span><span class="sxs-lookup"><span data-stu-id="3c595-161">View or clear the history of an RFM analysis record</span></span>
<span data-ttu-id="3c595-162">RFM analizi kaydı geçmişini görüntülemek veya silmek için bu yordamı kullanın.</span><span class="sxs-lookup"><span data-stu-id="3c595-162">Use this procedure to view or clear the history of an RFM analysis record.</span></span> 

1.  <span data-ttu-id="3c595-163">**Çağrı merkezi** > **Periyodik** > **RFM analizi**'ne gidin.</span><span class="sxs-lookup"><span data-stu-id="3c595-163">Go to **Call center** > **Periodic** > **RFM analysis**.</span></span>

2.  <span data-ttu-id="3c595-164">**RFM analizi** sayfasında, görüntülemek istediğiniz kaydı seçin.</span><span class="sxs-lookup"><span data-stu-id="3c595-164">On the **RFM analysis** page, select the record that you want to view.</span></span>

3.  <span data-ttu-id="3c595-165">Kayıt geçmişini görüntülemek için **Geçmiş** hızlı sekmesini seçin.</span><span class="sxs-lookup"><span data-stu-id="3c595-165">To view the record history, select the **History** FastTab.</span></span>

4.  <span data-ttu-id="3c595-166">Kayıt geçmişini silmek için **Geçmişi temizle**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="3c595-166">To clear the history of the record, select **Clear history**.</span></span>
