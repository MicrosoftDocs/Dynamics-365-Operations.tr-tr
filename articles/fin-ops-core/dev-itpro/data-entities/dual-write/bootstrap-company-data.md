...
ms.locfileid: "3020064"
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="1cf6a-103">Şirket verileriyle önyükleme ile ilgili SSS</span><span class="sxs-lookup"><span data-stu-id="1cf6a-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]

[!include [preview-banner](../../includes/preview-banner.md)]

## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="1cf6a-104">Neden önyükleme yapmam gerekiyor?</span><span class="sxs-lookup"><span data-stu-id="1cf6a-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="1cf6a-105">İş verileriyle birlikte mevcut bir Common Data Service veya diğer Dynamics 365 uygulaması kurulumunuz olabilir ve bunun için çift yazma bağlantısını etkinleştirmek isteyebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="1cf6a-106">Bu durumda, çift yazma bağlantısını etkinleştirmeden önce şirket bilgileriyle birlikte Common Data Service veya diğer Dynamics 365 uygulamasını önyüklemeniz gerekebilir.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="1cf6a-107">Önyüklemeyi ne zaman kullanmalıyım?</span><span class="sxs-lookup"><span data-stu-id="1cf6a-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="1cf6a-108">Çift yazma varlık eşlemelerini etkinleştirmeden önce önyüklemeyi kullanmanız gerekir (5. adımda).</span><span class="sxs-lookup"><span data-stu-id="1cf6a-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="1cf6a-109">Finance and Operations uygulamanız ve Common Data Service veya diğer Dynamics 365 uygulamanızın kurulumları arasında çift yazma bağlantısını ayarlamak için Finance and Operations uygulamasında yönetici olarak oturum açın.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="1cf6a-110">**Veri Yönetimi** modülüne gidin ve **Çift Yazma** düğmesine tıklayın.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="1cf6a-111">Bu, **Veri Tümleştirici**'yi başlatır.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="1cf6a-112">Bir veya daha fazla şirket için çift yazma bağlantısı oluşturun.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="1cf6a-113">![Çift yazma bağlantısı oluşturma](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="1cf6a-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="1cf6a-114">**Cdm_companies** varlık eşlemesini etkinleştirin.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="1cf6a-115">Bu, Finance and Operations uygulamasından şirketleri Common Data Service'e eşitler.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="1cf6a-116">![Varlık eşlemesini etkinleştirme](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="1cf6a-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="1cf6a-117">Common Data Service veya diğer Dynamics 365 uygulaması kurulumunda örnek önyükleme kodunu çalıştırın.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="1cf6a-118">Önyükleme tamamlandığında ve sistem canlı eşitlemeye hazır olduğunda varlık eşlemelerini etkinleştirin.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="1cf6a-119">Varlık eşlemelerinin etkinleştirilmesi, etkin varlık eşlemeleri için ilk veri eşitlemeyi tetikler.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="1cf6a-120">Çift yazma bağlantısında seçilen şirketlere karşılık gelen veriler, Finance and Operations uygulaması ile Common Data Service arasında eşitlenir.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="1cf6a-121">Kod örneğini nasıl kullanırım?</span><span class="sxs-lookup"><span data-stu-id="1cf6a-121">How to I use the code sample?</span></span>
<span data-ttu-id="1cf6a-122">Örnek kod, Visual Studio'da yükleyebileceğiniz bir C# uygulamasıdır.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="1cf6a-123">Common Data Service SDK'sında NuGet paketi bağımlılıklarını alır, bunu standart Visual Studio araçları aracılığıyla yenileyebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="1cf6a-124">Çözümü Visual Studio'da çıkarıp açtıktan ve NuGet paketlerini geri yükledikten sonra kodda **TODO** için arama yapın.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="1cf6a-125">Şirket bilgilerini nasıl önyüklemek istediğinizle ilgili vermeniz gereken her karar, bir kural uygulamasının örnek koduyla **TODO** tarafından not edilir.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="1cf6a-126">Örnek kod, şirkete göre varlık kayıtlarını sınıflandırmanızın çeşitli yollarından yalnızca birini gösterir.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="1cf6a-127">**TODO** bölümlerindeki mantığı değiştirerek özel sınıflandırmalar oluşturabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="1cf6a-128">Ne beklemeliyim?</span><span class="sxs-lookup"><span data-stu-id="1cf6a-128">What should I expect?</span></span>
<span data-ttu-id="1cf6a-129">Örnek uygulama, varsayılan olarak iş birimi ile şirket kodu arasındaki eşlemelerinin sözlüğünü sağlamanıza olanak tanır.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="1cf6a-130">**OwningBusinessUnit** alanında önyükleme yaptığınız her varlık, otomatik olarak belirtilen şirketi kullanacak şekilde ayarlanır.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="1cf6a-131">Ürün gibi **OwningBusinessUnit** alanı olmayan her varlık, şirketi boş bir iş birimi değeri ile eşlenecek şekilde ayarlar.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="1cf6a-132">Konsol uygulaması bir parametrenin **–simulate** veya **–apply** olmasını bekler.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="1cf6a-133">**–simulate** komut satırı parametresini kullanırsanız hiçbir veri güncelleştirilmez.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="1cf6a-134">Güncelleştirilecek her varlık için bir tane olacak şekilde yalnızca **simulation_<entityname>.csv** dosyaları araçla aynı dizinde oluşturulur.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="1cf6a-135">Kodun şirket değerlerini beklendiği gibi güncelleştirmesini sağlamak için çalışırken bu dosyaları tekrar gözden geçirebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="1cf6a-136">Benzetimi yapılan güncelleştirmeleri tamamladığınızda **–apply** parametresini kullanın.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="1cf6a-137">Bu, o anda yanlış şirket değeri bulunan tüm kayıtları aynı anda toplam 1000 kayıt içerecek şekilde toplu olarak güncelleştirir (varsayılan olarak).</span><span class="sxs-lookup"><span data-stu-id="1cf6a-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="1cf6a-138">Kod bir kez etkili olacak şekilde sağlanır. Bu, kodu yeniden çalıştırabileceğiniz ve yalnızca yanlış atanan şirketlerin güncelleştirileceği anlamına gelir.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="1cf6a-139">Kod, **–apply** ile çalıştırılırken, **applied_<entityname>.csv** olarak adlandırılan CSV dosyalarında yapılan değişiklikleri çıktı olarak verir.</span><span class="sxs-lookup"><span data-stu-id="1cf6a-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
