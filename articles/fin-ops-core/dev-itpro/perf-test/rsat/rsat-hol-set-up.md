...
### <a name="user-eligibility"></a><span data-ttu-id="c6ab4-124">Kullanıcı uygunluğu</span><span class="sxs-lookup"><span data-stu-id="c6ab4-124">User eligibility</span></span>
...
<span data-ttu-id="c6ab4-173">LCS projeniz için varolan bir müşteri uygulamasını veya ortak projeyi kullanabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-173">For your LCS project, you can use an existing customer implementation or partner project.</span></span>
...
3. <span data-ttu-id="c6ab4-187">Sol bölmede **Visual Studio Team Services**'i seçin ve sonra **Setup Visual Studio Team Services**'i seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-187">In the left pane, select **Visual Studio Team Services**, and then select **Setup Visual Studio Team Services**.</span></span>
...
8. <span data-ttu-id="c6ab4-270">Görev kaydını yerel olarak kaydetmek ve daha sonra LCS yoluyla BPM ile yüklemek istiyorsanız şu adımları izleyin:</span><span class="sxs-lookup"><span data-stu-id="c6ab4-270">If you want to save the task recording locally and then upload it to BPM through LCS, follow these steps:</span></span>
    ...
    3. <span data-ttu-id="c6ab4-275">**RSAT** BPM kitaplığına gidin ve görev kaydını kaydedeceğiniz iş sürecini seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-275">Go to the **RSAT** BPM library, and select the business process to save the task recording against.</span></span>
...
5. <span data-ttu-id="c6ab4-316">**Eylem** menüsünde sertifikayı **Güvenilen Kök Sertifika Yetkilileri** konumuna yapıştırmak için **Yapıştır**'ı seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-316">On the **Action** menu, select **Paste** to copy the certificate to the **Trusted Root Certification Authorities** location.</span></span>
...
## <a name="get-started-with-rsat"></a><span data-ttu-id="c6ab4-352">RSAT kullanmaya başlama</span><span class="sxs-lookup"><span data-stu-id="c6ab4-352">Get started with RSAT</span></span>
...
### <a name="load-and-run-test-cases"></a><span data-ttu-id="c6ab4-418">Test çalışmalarını yükleme ve çalıştırma</span><span class="sxs-lookup"><span data-stu-id="c6ab4-418">Load and run test cases</span></span>
...
4. <span data-ttu-id="c6ab4-431">Otomasyon dosyalarının ve Azure DevOps parametre dosyalarının kaydedildiğini doğrulamak için Azure DevOps projesine gidin,**Panolar \> İş Maddesi**'ni seçin ve **Yeni ürün oluştur** test olayını seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-431">To verify that the automation files and parameter files are saved to Azure DevOps, go to the Azure DevOps project, select **Boards \> Work Items**, and select the **Create a new product** test case.</span></span> <span data-ttu-id="c6ab4-432">**Ekler** sekmesinde dört dosya görmelisiniz:</span><span class="sxs-lookup"><span data-stu-id="c6ab4-432">On the **Attachments** tab, you should see four files:</span></span>
    ...
    - <span data-ttu-id="c6ab4-433">**.cs** – C\# otomasyon dosyası</span><span class="sxs-lookup"><span data-stu-id="c6ab4-433">**.cs** – C\# automation file</span></span>
...
2. <span data-ttu-id="c6ab4-459">Hata iletisinin tamamını görüntülemek için **Ayrıntıları** seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-459">Select **Details** to view the whole error message.</span></span>

    ![Tüm hata iletisi](./media/setup_rsa_tool_74.png)

3. <span data-ttu-id="c6ab4-461">Hata iletisinin ayrıntılı bir sürümünü Azure DevOps'ta görüntülemek için **Azure DevOps'da Aç**'ı seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-461">To view a detailed version of the error message in Azure DevOps, select **Open in Azure DevOps**.</span></span> <span data-ttu-id="c6ab4-462">Azure DevOps'ta, test olayının durumunu ve ayrıntılı hata iletisini görebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-462">In Azure DevOps, you can view the status of the test case and the detailed error message.</span></span>

    ![Azure DevOps'ta ayrıntılı hata iletisi](./media/setup_rsa_tool_75.png)

4. <span data-ttu-id="c6ab4-464">Azure DevOps test sonuçlarını doğrudan proje içinde görüntülemek için,**Test Planları \> Test Planları \> Çalıştır**'a gidin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-464">To view the test results directly in the Azure DevOps project, go to **Test Plans \> Test Plans \> Runs**.</span></span> <span data-ttu-id="c6ab4-465">Daha fazla ayrıntı görmek istediğiniz test olayına çift tıklayın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-465">Double-click the test run that you want to see more details for.</span></span>

    ![Azure DevOps'ta test çalışmalarının listesi](./media/setup_rsa_tool_76.png)

5. <span data-ttu-id="c6ab4-467">**Çalıştırma özeti** sekmesi test olayının başarısız olduğunu gösterir, ancak gerçek hata iletisini sağlamaz.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-467">The **Run summary** tab indicates that the test case failed, but it doesn't provide the actual error message.</span></span> <span data-ttu-id="c6ab4-468">Ayrıntılı hata iletisini görüntülemek için **Test sonuçları** sekmesini seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-468">To view the detailed error message, select the **Test results** tab.</span></span>

    ![Çalıştırma özeti sekmesi](./media/setup_rsa_tool_77.png)

    <span data-ttu-id="c6ab4-470">**Test sonuçları** sekmesi, sonuç ve hata iletisiyle birlikte test olayı bilgilerini sağlar.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-470">The **Test results** tab provides the test case information, together with the outcome and the error message.</span></span>

    ![Test sonuçları sekmesi](./media/setup_rsa_tool_78.png)

6. <span data-ttu-id="c6ab4-472">Ayrıntılı hata iletisini görüntülemek için ilgili kayda çift tıklayın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-472">Double-click the relevant record to view the detailed error message.</span></span>

    ![Ayrıntılı hata iletisi](./media/setup_rsa_tool_79.png)

    > [!NOTE]
    > <span data-ttu-id="c6ab4-474">Tüm hata iletileri yerel olarak **C:\\Users\\\$YourUserName\\AppData\\Roaming\\regressionTool\\errormsg-.txt**'te de bulunur.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-474">All error messages are also available locally in **C:\\Users\\\$YourUserName\\AppData\\Roaming\\regressionTool\\errormsg-.txt**.</span></span>

7. <span data-ttu-id="c6ab4-475">Test olayı sonuçlarını, **Dışa aktar**'ı seçerek test planı düzeyinden de dışa aktarabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-475">You can also export the test run results from the test plan level by selecting **Export**.</span></span>

    ![Test planını dışa aktarma](./media/setup_rsa_tool_80.png)

### <a name="modify-the-excel-parameter-file"></a><span data-ttu-id="c6ab4-477">Excel parametre dosyasını değiştirme</span><span class="sxs-lookup"><span data-stu-id="c6ab4-477">Modify the Excel parameter file</span></span>

1. <span data-ttu-id="c6ab4-478">RSAT'ı açın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-478">Open RSAT.</span></span>
2. <span data-ttu-id="c6ab4-479">Test olayını seçin ve sonra Excel parametre dosyasını açmak için **Düzenle**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-479">Select the test case, and then select **Edit** to open the Excel parameter file.</span></span>

    <span data-ttu-id="c6ab4-480">**EcoResProductCreate** tablosunda, **Ürün numarası** alanı değerinin sabit kodlanmış olduğuna dikkat edin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-480">On the **EcoResProductCreate** sheet, notice that the value of the **Product number** field is hard-coded.</span></span> <span data-ttu-id="c6ab4-481">Test olayını yeniden çalıştırmadan önce bu alanı yeni bir ürün numarasıyla güncelleştirmelisiniz.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-481">You must update this field to a new product number before you run the test case again.</span></span>

3. <span data-ttu-id="c6ab4-482">Excel parametre dosyasını yeniden açmak ve her seferinde ürün numarasını el ile güncelleştirmek zorunda kalmadan her çalışma için benzersiz bir ürün numarası oluşturmak için aşağıdaki Excel formülünü kullanın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-482">To generate a unique product number for each run without having to reopen the Excel parameter file and manually update the product number every time, use the following Excel formula.</span></span>
    ...
    > [!NOTE]
    > <span data-ttu-id="c6ab4-483">**Genel** sekmesine ek olarak the Excel parametre dosyası, test olayının ziyaretlerinin her form sayfası için bir veri sekmesi içerir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-483">In addition to the **General** tab, the Excel parameter file contains a data tab for every form page the test case visits.</span></span>

    ![Ürün numarası dosyası](./media/setup_rsa_tool_81.png)

4. <span data-ttu-id="c6ab4-485">**Kaydet**'i seçip Excel .çalışma sayfasını kapatın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-485">Select **Save**, and then close the Excel workbook.</span></span>
5. <span data-ttu-id="c6ab4-486">Excel parametre dosyasını Azure DevOps'a kaydetmek için **Yükle**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-486">Select **Upload** to save the Excel parameter file to Azure DevOps.</span></span>

    ![İleti yükleme başarılı](./media/setup_rsa_tool_82.png)

    > [!NOTE]
    > <span data-ttu-id="c6ab4-488">Test olaylarını belirli bir kullanıcı bağlamında çalıştırmak için, kullanıcının e-posta kodunu Excel parametre dosyasının **Genel** sekmesindeki **Test Kullanıcısı** alanına girin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-488">To run test cases in a specific user context, enter the user's email ID in the **Test User** field on the **General** tab of the Excel parameter file.</span></span> <span data-ttu-id="c6ab4-489">RSAT'ın en son sürümünde, Excel parametre dosyasındaki alanların düzeni güncelleştirildi, ancak kavram aynı kaldı.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-489">In the latest version of RSAT, the layout of the fields in the Excel parameter file has been updated, but the concept remains the same.</span></span>
    >
    > ![Test Kullanıcısı alanı](./media/setup_rsa_tool_83.png)

### <a name="validate-the-results"></a><span data-ttu-id="c6ab4-491">Sonuçları doğrulama</span><span class="sxs-lookup"><span data-stu-id="c6ab4-491">Validate the results</span></span>

- <span data-ttu-id="c6ab4-492">Test olayını yeniden çalıştırmak için **Çalıştır**'ı seçin ve test olayının geçtiğini doğrulayın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-492">Select **Run** to rerun the test case, and verify that the test case has passed.</span></span> <span data-ttu-id="c6ab4-493">Test sonuçlarını, [View the test results](#view-the-test-results)'ta tanımlandığı gibi görüntüleyebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-493">You can view the test results as described in the [View the test results](#view-the-test-results) section.</span></span>

    ![Sonuç alanı Başarılı olarak ayarlandı](./media/setup_rsa_tool_84.png)

### <a name="chaining-of-test-cases"></a><span data-ttu-id="c6ab4-495">Test olaylarının zincirlenmesi</span><span class="sxs-lookup"><span data-stu-id="c6ab4-495">Chaining of test cases</span></span>

<span data-ttu-id="c6ab4-496">RSAT'ın en önemli özelliklerinden biri test olaylarının zincirlenmesidir (bir başka deyişle, bir testin diğer testlere geçiş yeteneği).</span><span class="sxs-lookup"><span data-stu-id="c6ab4-496">One key feature of RSAT is the chaining of test cases (that is, the capability of a test to pass values to other tests).</span></span> <span data-ttu-id="c6ab4-497">Test olayları, Azure DevOps test planındaki tanımlı sıralara göre çalıştırılır.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-497">Test cases are run according to their defined order in the Azure DevOps test plan.</span></span> <span data-ttu-id="c6ab4-498">(Bu sıra test aracının kendisinde de güncelleştirilebilir.) Bu nedenle, değişkenleri bir test olayından diğerine geçirmek istiyorsanız testlerin doğru sırada olması çok önemlidir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-498">(This order can also be updated in the test tool itself.) Therefore, if you want to pass variables from one test case to another, it's very important that the tests be in the correct order.</span></span>

<span data-ttu-id="c6ab4-499">Bu bölümde, ilk test olayında bir kaydedilmiş değişken oluşturacak, ikinci bir test olayı yaratmayacak ve kaydedilmiş değişkeni ilk test olayından ikinci test olayına geçireceksiniz.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-499">In this section, you will create a saved variable in the first test case, create a second test case, and pass the saved variable from the first test case to the second test case.</span></span> <span data-ttu-id="c6ab4-500">Daha sonra test olaylarını RSAT'da zincirleme bir test olayı olarak çalıştırırsınız.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-500">You will then run the test cases as a chained test case in RSAT.</span></span>

#### <a name="modify-an-existing-task-recording-to-create-a-saved-variable"></a><span data-ttu-id="c6ab4-501">Kaydedilmiş bir değişken oluşturmak için varolan görev kaydını değiştirme</span><span class="sxs-lookup"><span data-stu-id="c6ab4-501">Modify an existing task recording to create a saved variable</span></span>

1. <span data-ttu-id="c6ab4-502">İstemciyi açın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-502">Open the client.</span></span>
2. <span data-ttu-id="c6ab4-503">**Ayarlar** düğmesini (çark simgesi) seçin ve sonra **Görev kaydedici**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-503">Select the **Settings** button (the gear symbol), and then select **Task recorder**.</span></span>
3. <span data-ttu-id="c6ab4-504">**Kaydı Düzenle**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-504">Select **Edit Recording**.</span></span>

    ![Kaydı Düzenle düğmesi](./media/setup_rsa_tool_85.png)

4. <span data-ttu-id="c6ab4-506">**Lifecycle Services'tan Aç**'ı seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-506">Select **Open from Lifecycle Services**.</span></span>

    ![Lifecycle Services'tan aç düğmesi](./media/setup_rsa_tool_86.png)

5. <span data-ttu-id="c6ab4-508">**Lifecycle Services kitaplığı**'nı seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-508">Select **Select the Lifecycle Services library**.</span></span>

    ![Lifecycle Services kitaplığını seç düğmesi](./media/setup_rsa_tool_87.png)

    <span data-ttu-id="c6ab4-510">BPM kitaplıkları LCS'den yüklenir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-510">BPM libraries are loaded from LCS.</span></span>

    ![BPM kitaplıkları yükleniyor](./media/setup_rsa_tool_88.png)

6. <span data-ttu-id="c6ab4-512">BPM kitaplıkları LCS'den yüklendikten sonra **RSAT** BPM kitaplığını seçin ve görev kaydının ilişkilendirildiği **Yeni bir ürün oluştur** iş süreci oluşturun.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-512">After the BPM libraries are loaded from LCS, select the **RSAT** BPM library and the **Create a new product** business process that the task recording was associated with.</span></span> <span data-ttu-id="c6ab4-513">Daha sonra **Tamam**'ı seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-513">Then select **OK**.</span></span>

    ![BPM kitaplığı ve iş süreci seçme](./media/setup_rsa_tool_89.png)

7. <span data-ttu-id="c6ab4-515">Uygun görev kaydının adı **Kayıt adı** alanına girilir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-515">The name of the appropriate task recording is entered in the **Recording name** field.</span></span> <span data-ttu-id="c6ab4-516">**Başlat**'ı seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-516">Select **Start**.</span></span>

    ![Kayıt adı alanına görev kaydı için benzersiz bir ad girin.](./media/setup_rsa_tool_90.png)

8. <span data-ttu-id="c6ab4-518">**Ürün bilgileri yönetimi \> Ürünler**'e gidin ve **Ürün oluştur**'un kaydedildiği orijinal görev kaydetmede **Yeni**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-518">Go to **Product information management \> Products**, and select **New** to open the page where the original task recording, **Create a product**, was recorded.</span></span>
9. <span data-ttu-id="c6ab4-519">**Adım ekle**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-519">Select **Insert step**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="c6ab4-520">Yeni adım, bölmesinde seçtiğiniz adımdan **sonra** eklenir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-520">The new step is inserted **after** the step that you selected in the pane.</span></span>

    ![Adım ekle düğmesi](./media/setup_rsa_tool_91.png)

10. <span data-ttu-id="c6ab4-522">**Ürün numarası** alanına sağ tıklayın ve sonra **Görev kaydedici \> Kopyala**'yı seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-522">Right-click the **Product number** field, and then select **Task recorder \> Copy**.</span></span>

    ![Komutu kopyalama](./media/setup_rsa_tool_92.png)

11. <span data-ttu-id="c6ab4-524">Yeni adım bölmeye eklenir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-524">A new step is added in the pane.</span></span> <span data-ttu-id="c6ab4-525">Daha sonra gereksinim duyacağınız **Ürün numarası** alanındaki değeri not edin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-525">Make a note of the value in the **Product number** field, because you will need it later.</span></span>

    ![Yeni adım eklendi](./media/setup_rsa_tool_93.png)

12. <span data-ttu-id="c6ab4-527">**Düzenlemeyi bitir**'i seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-527">Select **Done editing**.</span></span>
13. <span data-ttu-id="c6ab4-528"> **Save to Lifecycle Services**'e kaydeti seçin ve yeni görev kaydını, özgün görev kaydının ilişkilendirildiği aynı BPM kitaplığıyla ve iş süreciyle ilişkilendirin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-528">Select **Save to Lifecycle Services**, and associate the new task recording with the same BPM library and business process that the original task recording was associated with.</span></span> <span data-ttu-id="c6ab4-529">Daha fazla bilgi için [Görev kaydı oluşturma ve bunu BPM kitaplığına kaydetme](#create-a-task-recording-and-save-it-to-the-bpm-library)bölümüne bakın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-529">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>
14. <span data-ttu-id="c6ab4-530">BPM kitaplığına gidin ve **Test olaylarını eşitle**'yi [BPM ile Azure DevOps eşitlemesini test etme](#test-the-synchronization-from-bpm-to-azure-devops) bölümünde açıklandığı gibi Azure DevOps'a iliştirin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-530">Go to the BPM library, and select **Sync testcases** to overwrite the task recording that is attached to the test case in Azure DevOps, as described in the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>
15. <span data-ttu-id="c6ab4-531">RSAT'ı açın ve test paketindeki tüm test olaylarını yeniden yüklemek için **Yükle**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-531">Open RSAT, and select **Load** to reload all the test cases in the test suite.</span></span> <span data-ttu-id="c6ab4-532">Test olayını seçerek ve [Test olaylarını yükleme ve çalıştırma](#load-and-run-test-cases) bölümünde açıklandığı gibi **Yeni \> Test Yürütme ve Parametre dosyaları**'nı seçerek uygun test olayı için otomasyon ve parametre dosyalarını yeniden oluşturmalısınız.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-532">You must regenerate the automation and parameter files for the appropriate test case by selecting the test case and then selecting **New \> Generate Test Execution and Parameter files**, as described in the [Load and run test cases](#load-and-run-test-cases) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="c6ab4-533">Excel parametre dosyası açık bırakılırsa yeniden oluşturma işlemi başarısız olur.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-533">If the Excel parameter file was left open, regeneration will fail.</span></span> <span data-ttu-id="c6ab4-534">Bu nedenle, yeni Excel parametre dosyasını oluşturmadan önce test olayı için Excel parametre dosyasının kapalı olduğundan emin olun.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-534">Therefore, make sure that the Excel parameter file for the test case is closed before you generate the new Excel parameter file.</span></span>

16. <span data-ttu-id="c6ab4-535">Yeni Excel parametre dosyasını açmak için **Düzenle**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-535">Select **Edit** to open the new Excel parameter file.</span></span> <span data-ttu-id="c6ab4-536">Çevrimiçi 9 yeni bir **Kaydedilmiş değişken** girişi göreceksiniz.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-536">You will see a new **Saved variable** entry on line 9.</span></span> <span data-ttu-id="c6ab4-537">Bu değişken **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**, görev kaydının XML dosyasına kaydedilir ve sonraki testlerde kullanılabilir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-537">This variable, **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**, is saved in the task recording's XML file and can be used in subsequent tests.</span></span>

    ![Kaydedilen değişken girişi](./media/setup_rsa_tool_94.png)

#### <a name="create-a-new-test-case"></a><span data-ttu-id="c6ab4-539">Yeni bir test olayı oluşturma</span><span class="sxs-lookup"><span data-stu-id="c6ab4-539">Create a new test case</span></span>

1. <span data-ttu-id="c6ab4-540">**RSAT** BPM kitaplığına gidin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-540">Go to the **RSAT** BPM library.</span></span>
2. <span data-ttu-id="c6ab4-541">**Örnek Desteği İş Süreci** sürecini seçin ve sağ taraftaki **Düzenle**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-541">Select the **Sample Support Business Process** process, and then, on the right, select **Edit mode**.</span></span>
3. <span data-ttu-id="c6ab4-542">Hem **Ad** alanını hem de **Açıklama** alanını **Ürünü serbest bırak** olarak değiştirin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-542">Change the value of both the **Name** field and the **Description** field to **Release a product**.</span></span> <span data-ttu-id="c6ab4-543">Sonra **Kaydet**'i seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-543">Then select **Save**.</span></span>

    ![Ad ve açıklama bir ürünü serbest bırakmak için değişti](./media/setup_rsa_tool_95.png)

#### <a name="create-a-new-task-recording-that-has-a-validate-function"></a><span data-ttu-id="c6ab4-545">Doğrulama işlevi olan yeni bir görev kaydı oluşturma</span><span class="sxs-lookup"><span data-stu-id="c6ab4-545">Create a new task recording that has a Validate function</span></span>

- <span data-ttu-id="c6ab4-546">Daha önce USRT tüzel kişiliği tarafından oluşturulan ürünü serbest bırakmak için bir görev kaydı oluşturun.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-546">Create a task recording to release the product that was created earlier to the USRT legal entity.</span></span> <span data-ttu-id="c6ab4-547">Daha fazla bilgi için [Görev kaydı oluşturma ve bunu BPM kitaplığına kaydetme](#create-a-task-recording-and-save-it-to-the-bpm-library)bölümüne bakın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-547">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="c6ab4-548">Zincirleme test olayları için, *alan değerini el ile yazarak* gereksinim duyduğunuz kaydı bulmanız veya filtreleyerek her zaman önerilir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-548">For chained test cases, we always recommend that you find or filter for the record that you require by *manually typing the value of the field*.</span></span> <span data-ttu-id="c6ab4-549">Bu şekilde, söz konusu eylem, sonraki test olayında dikkate alınması gereken kaydı belirleyebilir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-549">In that way, the tool can determine the record that the action must be taken against in the subsequent test case.</span></span>

    ![Doğrulama işlevi olan yeni bir görev kaydı](./media/setup_rsa_tool_96.png)

    <span data-ttu-id="c6ab4-551">Yukarıdaki resimde gösterildiği gibi ürün Quick Filter'la bulunduktan sonra ancak **Ürünleri serbest bırak**'ı seçmeden önce, **Ürün numarası** alanının değerini doğrulamak için ürün kimliğinin daha önce oluşturulmuş ürün kimliği olduğundan emin olun.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-551">As the preceding illustration shows, after the product is found by using the Quick Filter, but before you select **Release products**, you validate the value of the **Product number** field to make sure that the product ID is the product ID that was created earlier.</span></span> <span data-ttu-id="c6ab4-552">Değeri doğrulamak için **ürün numarası** alanına sağ tıklayın ve **Görev kaydedici \> Doğrula \> Geçerli Değer**'i seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-552">To validate the value, right-click the **Product number** field, and then select **Task recorder \> Validate \> Current Value**.</span></span>

    ![Geçerli kaydı doğrulama](./media/setup_rsa_tool_97.png)

#### <a name="save-the-task-recording-to-bpm"></a><span data-ttu-id="c6ab4-554">Görev kaydetmeyi BPM'e kaydetme</span><span class="sxs-lookup"><span data-stu-id="c6ab4-554">Save the task recording to BPM</span></span>

1. <span data-ttu-id="c6ab4-555">Görev kaydı tamamlandıktan sonra **Lifecycle Services'e kaydet**'i seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-555">After the task recording is completed, select **Save to Lifecycle Services**.</span></span>

    ![Tamamlanan görev kaydını Lifecycle Services'e kaydetme](./media/setup_rsa_tool_98.png)

2. <span data-ttu-id="c6ab4-557">Kitaplık bilgileri LCS'den yüklenir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-557">Library information is loaded from LCS.</span></span>

    ![LCS'den kitaplık bilgileri yükleniyor](./media/setup_rsa_tool_99.png)

3. <span data-ttu-id="c6ab4-559">Görev kaydını ilişkilendirmek için BPM kitaplığını seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-559">Select the BPM library to associate the task recording with.</span></span> <span data-ttu-id="c6ab4-560">Bu öğretici için, yukarıda oluşturulmuş **RSAT** BPM kitaplığını seçin ve altında **Ürünü serbest bırakma** iş sürecini seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-560">For this tutorial, select the **RSAT** BPM library that was created earlier and the **Release a product** business process under it.</span></span> <span data-ttu-id="c6ab4-561">Daha sonra **Tamam**'ı seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-561">Then select **OK**.</span></span>

#### <a name="sync-bpm-to-azure-devops"></a><span data-ttu-id="c6ab4-562">BPM'i Azure DevOps ile eşitleme</span><span class="sxs-lookup"><span data-stu-id="c6ab4-562">Sync BPM to Azure DevOps</span></span>

1. <span data-ttu-id="c6ab4-563">BPM kitaplığına gidin ve **RSAT** kitaplığını açın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-563">Go to the BPM library, and open the **RSAT** library.</span></span>
2. <span data-ttu-id="c6ab4-564">**VSTS eşitleme**'yi seçin ve sonra **Test olaylarını eşitle**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-564">Select **VSTS sync** and then **Sync test cases**.</span></span> <span data-ttu-id="c6ab4-565">Daha fazla bilgi için [BPM ile Azure DevOps eşitlemesini test etme](#test-the-synchronization-from-bpm-to-azure-devops) bölümüne bakın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-565">For more information, see the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>

    <span data-ttu-id="c6ab4-566">Eşitleme tamamlandıktan sonra yeni iş maddesi ve **Bir ürünü serbest bırakma**'ya karşılık gelen test olayı **Panolar \> İş Maddeleri**'ndeki .Azure DevOps'da görünür</span><span class="sxs-lookup"><span data-stu-id="c6ab4-566">After the synchronization is completed, the new work item and the corresponding test case for the **Release a product** business process appear in Azure DevOps at **Boards \> Work Items**.</span></span>

#### <a name="add-the-new-test-case-to-the-existing-test-suite"></a><span data-ttu-id="c6ab4-567">Varolan test paketine yeni test çalışmasını ekleme</span><span class="sxs-lookup"><span data-stu-id="c6ab4-567">Add the new test case to the existing test suite</span></span>

1. <span data-ttu-id="c6ab4-568">**Test planları \> Test planları**'na gidin ve **RSAT Test Planı** planını seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-568">Go to **Test plans \> Test plans**, and select the **RSAT Test Plan** plan.</span></span>
2. <span data-ttu-id="c6ab4-569">**Varolanı ekle**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-569">Select **Add existing**.</span></span>
3. <span data-ttu-id="c6ab4-570">**Test olaylarını pakete ekle** sayfasında **Sorguyu çalıştır**'ı seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-570">On the **Add test cases to suite** page, select **Run query**.</span></span>
4. <span data-ttu-id="c6ab4-571">**Bir ürünü serbest bırakmak** için oluşturulan yeni test olayını seçin ve sonra sayfanın sağ alt köşesinde **Test olayları ekle**'yi seçin (Bu düğme aşağıdaki çizimde gösterilmez).</span><span class="sxs-lookup"><span data-stu-id="c6ab4-571">Select the new test case that was created for **Release a product**, and then select **Add test cases** in the lower-right corner of the page (this button isn't shown in the following illustration).</span></span>

    ![Paket sayfasına test durumları ekleme](./media/setup_rsa_tool_100.png)

    <span data-ttu-id="c6ab4-573">Test paketinin şimdi iki test olayı vardır.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-573">The test suite now has two test cases.</span></span>

    ![Test paketinin şimdi iki test olayı var](./media/setup_rsa_tool_101.png)

#### <a name="load-test-cases-into-rsat"></a><span data-ttu-id="c6ab4-575">RSAT'a test olaylarını yükleme</span><span class="sxs-lookup"><span data-stu-id="c6ab4-575">Load test cases into RSAT</span></span>

1. <span data-ttu-id="c6ab4-576">RSAT'ı açın ve **Yükle** yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-576">Open RSAT, and select **Load**.</span></span>
2. <span data-ttu-id="c6ab4-577">Test olayları yüklenir ve "Bu eylem Excel test veri dosyalarının üzerine yazacak, yerel değişikliklerin kaybolmasına neden olacak.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-577">The test cases are loaded, and you receive a warning that states, "This action will overwrite Excel test data files, local changes will be lost.</span></span> <span data-ttu-id="c6ab4-578">Devam etmek istiyor musunuz?" yazan bir uyarı alacaksınız</span><span class="sxs-lookup"><span data-stu-id="c6ab4-578">Do you want to proceed?"</span></span> <span data-ttu-id="c6ab4-579">Excel parametresi dosyalarını yerel sistemde güncelleştirmek ama Azure DevOps'a yüklenen Excel parametre dosyalarını güncelleştirmemek için **Evet**'i seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-579">Select **Yes** to update the Excel parameter files in the local system but not the Excel parameter files that were uploaded to Azure DevOps.</span></span>

    ![Bu eylem Excel test veri dosyalarının üzerine yazacak](./media/setup_rsa_tool_102.png)

    <span data-ttu-id="c6ab4-581">Her iki test olayı da, ilk test olayı için Excel parametre dosyası ile birlikte yüklenir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-581">Both the test cases are loaded, together with the Excel parameter file for the first test case.</span></span> <span data-ttu-id="c6ab4-582">Son çalıştırmada karşıya **Yükle**'yi seçtiğinizden, parametre dosyaları Azure DevOps'tan çekilir .</span><span class="sxs-lookup"><span data-stu-id="c6ab4-582">Because you selected **Upload** in the last run, the parameter files are pulled from Azure DevOps.</span></span>

    ![Test senaryoları yüklendi](./media/setup_rsa_tool_103.png)

3. <span data-ttu-id="c6ab4-584">Yalnızca ikinci test olayını seçin ve sonra  **Yeni \> Test Yürütmesi ve Parametre Dosyaları oluştur**'u seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-584">Select only the second test case, and then select **New \> Generate test execution and parameter files**.</span></span>

#### <a name="edit-the-excel-parameter-file"></a><span data-ttu-id="c6ab4-585">Excel parametre dosyasını düzenleme</span><span class="sxs-lookup"><span data-stu-id="c6ab4-585">Edit the Excel parameter file</span></span>

1. <span data-ttu-id="c6ab4-586">Yalnızca ikinci test olayını seçin ve sonra karşılık gelen Excel parametre dosyasını açmak için **Düzenle**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-586">Select only the second test case, and then select **Edit** to open the corresponding Excel parameter file.</span></span>
2. <span data-ttu-id="c6ab4-587">Kaydedilmiş değişken **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**'yi ilk test olayından ürün numarasının kullanıldığı tüm alanlara kopyalayın ([Kaydedilmiş bir değişken oluşturmak için varolan görev kaydını değiştirme](#modify-an-existing-task-recording-to-create-a-saved-variable) bölümüne bakın).</span><span class="sxs-lookup"><span data-stu-id="c6ab4-587">Copy the **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** saved variable (see the [Modify an existing task recording to create a saved variable](#modify-an-existing-task-recording-to-create-a-saved-variable) section) from the first test case into all the fields where the product number is used.</span></span> <span data-ttu-id="c6ab4-588">Bu durumda, değişkeni **Ürün numarasına** ve **Ürün Numarasını Doğrula** alanlarını **EcoResProductListPage** sayfasına kopyalarsınız.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-588">In this case, you copy the variable into the **Product number** and **Validate Product number** fields on the **EcoResProductListPage** sheet.</span></span>

    ![Ürün numarası ve ürün numarası doğrulama alanları](./media/setup_rsa_tool_104.png)

    > [!NOTE]
    > <span data-ttu-id="c6ab4-590">Değişkenler, yalnızca aynı test olayı sırasında testler arasında geçirilebilir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-590">Variables can be passed between tests only during the same test run.</span></span> <span data-ttu-id="c6ab4-591">Değişkenlerin adlarının tam olarak eşleşmesi gerekir.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-591">The names of the variables must match exactly.</span></span>

3. <span data-ttu-id="c6ab4-592">**Kaydet**'i seçip Excel .çalışma sayfasını kapatın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-592">Select **Save**, and then close the Excel workbook.</span></span>
4. <span data-ttu-id="c6ab4-593">Excel parametre dosyasında yaptığınız değişiklikleri kaydetmek için **Yükle**'yi seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-593">Select **Upload** to save the changes that you made to the Excel parameter file.</span></span>

#### <a name="run-the-chained-test-cases"></a><span data-ttu-id="c6ab4-594">Zincirleme test olaylarını çalıştırma</span><span class="sxs-lookup"><span data-stu-id="c6ab4-594">Run the chained test cases</span></span>

1. <span data-ttu-id="c6ab4-595">Çalıştırılacak iki test olayını da seçin ve sonra **Çalıştır**'ı seçin.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-595">Select both the test cases, and then select **Run**.</span></span>
2. <span data-ttu-id="c6ab4-596">Her iki test olayının da geçtiğini doğrulayın.</span><span class="sxs-lookup"><span data-stu-id="c6ab4-596">Verify that both test cases have passed.</span></span>
