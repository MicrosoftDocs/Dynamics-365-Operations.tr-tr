...
- Microsoft Dynamics 365 for Finance and Operations'taki, görev kaydedici özelliğinin Microsoft Dynamics Lifecycle Services (LCS) ve Microsoft Azure DevOps ile birlikte test çalıştırmalarını oluşturmanıza, yapılandırmanıza, çalıştırmanıza, araştırmanıza ve raporlamanıza olanak tanır.
- Finance and Operations'taki görev kaydediciyi kullanarak iş görevlerini kaydetmek ve sonra görev kayıtlarını kaynak kodu yazmak zorunda kalmadan otomatik bir test paketine dönüştürmek için işlevsel kullanıcıları güçlendirin.

## <a name="before-you-begin"></a>Başlamadan önce

### <a name="prerequisites"></a>Önkoşullar

- Bu öğretici, Microsoft Dynamics 365 for Finance and Operations sürüm 10.0 (Nisan 2019) ya da sonraki bir sürümü çalıştıran Finance and Operations ortamı için gereklidir. Microsoft Dynamics 365 for Finance and Operations Enterprise edition 7.3, Platform update 20 (PU20) ya da sonrasını kullanan müşteriler için desteklenmektedir.
- Kullanıcı, Finance and Operations ortamında yönetici haklarına sahip olmalıdır.
...
## <a name="finance-and-operations-environment"></a>Finance and Operations ortamı

### <a name="version-requirement"></a>Sürüm gereksinimi

10. sürümün çalıştığı bir Finance and Operations testi veya Sandbox ortamı gereklidir. 7.3, Platform update 20 (PU20) ya da sonrasını kullanan müşteriler için desteklenmektedir.

> [!NOTE]
> RSAT, bir web tarayıcısı Finance and Operations'ınız aracılığıyla test ortamına erişimi olmalıdır.
...
Bu adım, LCS ile ilgili olarak görev kayıtlarının Finance and Operations istemcisi aracılığıyla LCS içindeki uygun BPM kitaplığına kaydedilebileceği şekilde bağlanmak için gereklidir.

1. Finance and Operations müşterisi'ni açın.
...
> [!NOTE]
> Tüm görev kayıtlarınızın Finance and Operations ana panosunda göründüğünden emin olun. Tek kayıtların kısa kalmasını sağlayın ve satış siparişi oluşturma gibi bir iş görevine odaklanın.
...
RSAT, Windows 10 çalıştıran ve Finance and Operations ortamına bir web tarayıcısı üzerinden bağlanabilen herhangi bir bilgisayara kurulabilir (Internet Explorer ya da Google Chrome).
...
> [!NOTE]
> Finance and Operations ortamı bir katman 2 + ortamındaysa ortam için **Tüm** AOS bilgisayarları için wif.config dosyasında sertifika parmak izi güncelleştirilmelidir.
...
4. Aşağıdaki örnekte gösterildiği gibi, **wif.config** dosyasını sertifikanız ve yetki adınız için yeni bir yetki girişi ekleyerek günceştirin.
    ...
    > [!NOTE]
    > Birden fazla kullanıcı aynı Finance and Operations uygulamasını kullanıyorsa her kullanıcı ayrı parmak izleri oluşturmalı ve bu parmak izlerinin her biri **\<anahtarlar\>** bölümüne eklenmelidir.
...
Katman 2 + (Standart kabul testi sanal alanı veya daha ileri) Finance and Operations ortamı kullanılıyorsa RSAT'ın yüklendiği bilgisayarda aşağıdaki kayıt defteri ayarını doğrulayın veya ayarlayın. Bu adım, kimlik doğrulama veya RSAT bağlantı hatalarından kaçınmanıza yardımcı olacağından gereklidir.
...
3. Sağ üst köşedeki **Ayarlar** düğmesini (çark sembolü) seçin ve sonra görünen iletişim kutusunda aşağıdaki alanları doldurun:
    ...
    - **Ana bilgisayar adı** – Finance and Operations test ortamının ana bilgisayar adını **\<myaos\>.cloudax.dynamics.com** örneğindeki gibi girin. **https://** ya da **http://** öneki eklemeyin.
    - **SOAP Ana bilgisayar Adı** – Finance and Operations test ortamının SOAP ana bilgisayar adını girin. Genellikle, SOAP ana bilgisayar adı ana bilgisayar adıyla aynıdır, ancak **soap** soneki vardır. Örnek: **\<myaos\>soap.cloudax.dynamics.com**. **https://** ya da **http://** öneki eklemeyin.
    ...
    - **Yönetici kullanıcı adı** – Finance and Operations test ortamında bir yönetici kullanıcısının e-posta adresini girin.
    ...
    - **Şirket adı** – Excel parametre dosyaları oluşturulurken varsayılan Finance and Operations şirketi olarak kullanılacak şirket adını girin. Şirketi daha sonra Excel parametre dosyasını düzenleyerek değiştirebilirsiniz.
...
3. Excel parametre dosyasını yeniden açmak ve her seferinde ürün numarasını el ile güncelleştirmek zorunda kalmadan her çalışma için benzersiz bir ürün numarası oluşturmak için aşağıdaki Excel formülünü kullanın.
    ...
    > [!NOTE]
    > **Genel** sekmesine ek olarak the Excel parametre dosyası, test olayının ziyaretlerinin her Finance and Operations form sayfası için bir veri sekmesi içerir.
